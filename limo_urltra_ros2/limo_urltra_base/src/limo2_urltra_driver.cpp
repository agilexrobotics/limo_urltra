...
#include "limo_urltra_base/kinematics_model.hpp"

int flag=0; 

namespace AgileX {

    LimourltraDriver::LimourltraDriver(std::string node_name):rclcpp::Node(node_name),keep_running_(false){
        ...
        status_publisher_ = this->create_publisher<limo_urltra_msgs::msg::LimoUrltraStatus>("/limo2_status",50);
        imu_publisher_ = this->create_publisher<sensor_msgs::msg::Imu>("/imu",10);
    
        motion_cmd_sub_= this->create_subscription<geometry_msgs::msg::Twist>(
            "/cmd_vel",10,std::bind(&LimourltraDriver::twistCmdCallback,this,std::placeholders::_1));
    ...
    LimourltraDriver::~LimourltraDriver() {
    
    }

    void LimourltraDriver::connect(std::string dev_name, uint32_t bouadrate) {
        ...
        if (port_->openPort() == 0) {
            read_data_thread_ = std::shared_ptr<std::thread>(
                new std::thread(std::bind(&LimourltraDriver::readData, this)));
    ...
    void LimourltraDriver::readData() {
        uint8_t rx_data = 0;
        while (rclcpp::ok()) {
    ...
    void LimourltraDriver::processRxData(uint8_t data) {

        static LimourltraFrame frame;
    ...
    void LimourltraDriver::parseFrame(const LimourltraFrame& frame) {
        ...
        switch (frame.id) {
            ...
            case MSG_SYSTEM_STATE_ID: {
                ...
                publishLimourltraState(frame.stamp, vehicle_state, control_mode,
...
void LimourltraDriver::processErrorCode(uint16_t error_code) {
    ...
    void LimourltraDriver::enableCommandedMode() {
        LimourltraFrame frame;
    ...
    void LimourltraDriver::setMotionCommand(double linear_vel, double angular_vel,
    ...
    void LimourltraDriver::sendFrame(const LimourltraFrame& frame) {
        uint32_t checksum = 0;
    ...
    void LimourltraDriver::twistCmdCallback(const geometry_msgs::msg::Twist::SharedPtr msg)
    {
        double steer_cmd;
        double radius = 0.0;
        if (msg->linear.y != 0)
        ...
        switch (motion_mode_) {
            case MOTION_MODE_DUAL_ACKERMAN: {
                ...
                setMotionCommand(msg->linear.x, 0, 0, phi_i);
                break;
            }
    ...
    void LimourltraDriver::publishIMUData(double stamp) {
    ...
    void LimourltraDriver::publishOdometry(double stamp, double linear_velocity,
        ...
        last_time_ = current_time_;
            RCLCPP_INFO(this->get_logger(),"odom motion_mode_ :%u",motion_mode_);
...
void LimourltraDriver::publishLimourltraState(double stamp, uint8_t vehicle_state, uint8_t control_mode,
    double battery_voltage, uint16_t error_code, int8_t motion_mode) {

limo_urltra_msgs::msg::LimoUrltraStatus status_msg;
...
double LimourltraDriver::ConvertInnerAngleToCentral(double inner_angle) {
...
double LimourltraDriver::ConvertCentralAngleToInner(double central_angle) {
...
double inner_angle = std::atan(2 * wheelbase_ * std::sin(fabs(central_angle)) /
  ...
  inner_angle *= central_angle >= 0 ? 1.0 : -1.0;

return -inner_angle;
}
double LimourltraDriver::degToRad(double deg) {
    
    return deg / 180.0 * M_PI;
}

double LimourltraDriver::CalculateSteeringAngle(geometry_msgs::msg::Twist msg,double& radius) 
{
  double linear = std::abs(msg.linear.x);
  double angular = std::abs(msg.angular.z);

//   double wheelbase = LimourltraDriver::wheelbase_;
// Circular motion
...
void LimourltraDriver::SetMotionMode(uint8_t mode) {
...
double LimourltraDriver::computeSteeringAngle(double delta_fl_deg, double delta_fr_deg,
...
geometry_msgs::msg::Quaternion LimourltraDriver::createQuaternionMsgFromYaw(double yaw) {
  tf2::Quaternion q;
  q.setRPY(0, 0, yaw);
  return tf2::toMsg(q);
}
